...
struct Res {
    ...
    /// `<JSFunction name (sfi = 0x…)>` anywhere in a line, for the deferred
    /// TurboFan identity scan (Phase 8.1).
    func_any: Regex,
    /// `0x… <SharedFunctionInfo name>` anywhere in a line — the SFI address
    /// printed inside FrameState nodes.
    sfi_object: Regex,
}
...
impl TraceIndex {
    ...
    fn classify(&mut self, i: usize, text: &str) {
        ...
        if text.starts_with("-----") {
            ...
            match text.trim_end() {
                ...
                "--- Optimized code ---" if self.on_code_phase(i, "Optimized code") => {
                    return;
                }
    ...
    fn scan_identity(&mut self, line: &[u8]) {
        ...
        if self.code_section == Some(comp) {
            self.scan_code_identity(comp, line);
        } else if self.compilations[comp].key.tier == Tier::Turbofan {
    ...
    fn scan_tf_identity(&mut self, comp: usize, line: &[u8]) {
        if !contains_subslice(line, b"SharedFunctionInfo") && !contains_subslice(line, b"sfi = ") {
            return;
        }
        let stripped = ansi::strip(line);
        ...
        if sfi.is_none() {
            for c in r.func_any.captures_iter(text) {
                if c[1] == want {
                    sfi = Addr::parse(&c[2]);
                    break;
                }
    ...
    fn scan_code_identity(&mut self, comp: usize, line: &[u8]) {
        ...
        if let Some(rest) = text.strip_prefix(b"name = ") {
            ...
            if let Ok(kind) = std::str::from_utf8(rest) {
        ...
        // `name` and `kind` arrive on separate lines in either order; keep
        // scanning until the section closes (close_all clears the flag).
    }
...
",
    ...
    /// A different function's SFI object inside the body must not steal the
    /// identity; the name has to match the Begin line's.
    #[test]
    fn turbofan_identity_requires_the_matching_name() {
        let idx = index(
            "\
Begin compiling method add using TurboFan
----- Graph after V8.TFInlining -----
#5:HeapConstant[0x1 <JSFunction other (sfi = 0x99)>]()
#6:HeapConstant[0x2 <JSFunction add (sfi = 0x42)>]()
",
        );
        assert_eq!(idx.compilations[0].key.sfi, Addr(0x42));
    }
